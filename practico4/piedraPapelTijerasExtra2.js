...
PIEDRA, PAPEL O TIJERAS (EXTRA 2)
...
EXTRA 2
Agregar la posibilidad de que el usuario elija en cuantos jugadas quiere terminar
el juego, es decir:
   * una jugada: gana el ganador de esa única jugada
   * dos jugadas: gana quien gana ambas jugadas o puede haber un empate en caso
   de que el usuario gane una jugada y la computadora la otra
   * mejor de 3: quien gane 3 o 2 jugadas será el ganador
   * etc.
...
// Configuración
let jugadasMinimas = 1;
let jugadasMaximas = 10;
...
ejecutarPartida()
  ...
  obtenerJugadasPartida()
  ejecutarJugadas(jugadas)
    ejecutarJugada()
...
function ejecutarPartida() {
  ...
  // Obtenemos y validamos el número de jugadas de la partida
  let jugadasPartida = obtenerJugadasPartida();

  // Si el número de jugadas de la partida está definido (número de jugadas válido):
  // * ejecutamos las jugadas
  // * si se ejecutan todas las jugadas, imprimimos el resultado de la partida
  if (jugadasPartida !== undefined) {

    let jugadasEjecutadas = ejecutarJugadas(jugadasPartida);

    if (jugadasEjecutadas === jugadasPartida) {
...
function imprimirInstrucciones() {
  ...
  console.log('\n* El numero de jugadas de la partida es determinado por el usuario (desde %i hasta %i)', jugadasMinimas, jugadasMaximas);
  ...
  console.log('* Gana la partida quien gane mas jugadas');
}

function obtenerJugadasPartida() {
  // Obtiene y valida el número de jugadas de la partida a partir de un valor ingresado por consola
  // Para ser válido, el valor debe ser un número entero desde [jugadasMinimas] hasta [jugadasMaximas]
  // Si el valor no es válido:
  // * se imprime un mensaje de aviso
  // * se fuerza al usuario a intentar nuevamente o finalizar la partida
  // Tipo de dato de retorno (número de jugadas válido): number
  // Tipo de dato de retorno (no intentar nuevamente): undefined
  // Valor de retorno (número de jugadas válido): número entero desde [jugadasMinimas] hasta [jugadasMaximas]

  let jugadas = readlineSync.question('\nIngresa el numero de jugadas de la partida y presiona [Enter]: ');

  if (jugadas === '' || jugadas.search(/[^0-9]/) > -1 || Number(jugadas) < jugadasMinimas || Number(jugadas) > jugadasMaximas) {

    console.log('\nEl numero de jugadas debe ser un numero entero del %i al %i', jugadasMinimas, jugadasMaximas);

    // El usuario solo puede presionar las teclas [i] o [f]
  ...
  return Number(jugadas);
}

function ejecutarJugadas(jugadas) {
  // Ejecuta las jugadas de la partida
  // Mientras el número de jugadas por ejecutar es mayor que 0:
  // * incrementa el número de jugadas iniciadas
  // * imprime el número de jugada
  // * ejecuta la jugada
  // * si la jugada no está definida (jugada inválida), finaliza el ciclo
  // * incrementa la cantidad de jugadas ejecutadas
  // * disminuye la cantidad de jugadas por ejecutar
  // Tipo de dato de los parámetros: number
  // Valor que recibe: número entero desde [jugadasMinimas] hasta [jugadasMaximas]
  // Tipo de dato de retorno: number
  // Valor de retorno: número entero desde 0 hasta [jugadasMaximas]

  let jugadasIniciadas = 0;
  let jugadasEjecutadas = 0;

  while (jugadas > 0) {

    ++jugadasIniciadas;

    console.log('\nJUGADA %i', jugadasIniciadas);

    let jugada = ejecutarJugada();
...
function ejecutarJugada() {
  ...
  // Incrementa los contadores de las jugadas ganadas por el usuario y la computadora
  ...
  // * si el resultado es distinto de [EMPATE]:
  // * * incrementa [jugadasGanadasComputadora] o [jugadasGanadasUsuario], según corresponda
  // * si no:
  // * * ejecuta una nueva jugada
  // Tipo de dato de retorno (jugada válida): boolean
  // Tipo de dato de retorno (jugada inválida): undefined
  // Valor de retorno (jugada válida): true
...
function imprimirResultadoPartida() {
  // Imprime el resultado de la partida
  // Tipo de dato de retorno: undefined

  console.log('\nRESULTADO DE LA PARTIDA');
  console.log('\nJugadas ganadas por la computadora: %i', jugadasGanadasComputadora);
  console.log('Jugadas ganadas por el usuario: %i', jugadasGanadasUsuario);
